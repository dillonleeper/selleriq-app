...
function buildActions(comparisonAvailable: boolean, skuDrivers: SkuDriver[], inventoryRisks: InventoryRisk[]): ActionItem[] {
  ...
  for (const row of inventoryRisks) {
    ...
    candidates.push({
      ...
      reason: `Triggered because projected cover is ${daysOfCover.toFixed(1)} days including inbound, below the 28-day threshold.`,
      evidence: [`${n(row.available_quantity).toLocaleString()} available`, `${n(row.inbound_quantity).toLocaleString()} inbound`, `${recentUnits.toLocaleString()} units sold in 30 days`],
      impact,
      score: impact * confidenceWeight(confidence),
      confidence,
