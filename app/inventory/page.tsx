...
type FbaSortKey = 'sku' | 'total_fba' | 'avg_daily_units' | 'days_of_cover' | 'units_to_send'
  ...
  const fbaRows: FbaReplenRow[] = inventory
    .map(r => {
      const unitsToSend = r.avg_daily_units > 0
        ? Math.max(0, Math.round(FBA_TARGET_DAYS * r.avg_daily_units) - r.total_fba)
        : 0
      return {
        sku:            r.sku,
        title:          r.title,
          ...
          {tab === 'fba' && (
            <>
              ...
              <div style={{ fontSize: '12px', color: 'var(--text-muted)', marginBottom: '14px', padding: '10px 14px', background: 'var(--bg-card)', borderRadius: '8px', border: '1px solid var(--border)' }}>
                Shows SKUs where FBA inventory (fulfillable + inbound + reserved) is below <strong>{FBA_TARGET_DAYS} days</strong> of cover.
              ...
              <div style={{ display: 'flex', gap: '10px', marginBottom: '14px', alignItems: 'center' }}>
                ...
                <button onClick={() => exportCSV(
                  ['SKU', 'Title', 'Marketplace', 'Total FBA', 'Avg Daily Units', 'Days Cover', 'Units to Send', 'Urgency'],
                  fbaRows.map(r => [r.sku, `"${r.title}"`, r.marketplace, r.total_fba, r.avg_daily_units, r.days_of_cover ?? '', r.units_to_send, r.urgency]),
                    ...
                    <thead>
                      <tr>
                        ...
                        <th style={{ ...thSortable(fbaSortKey === 'total_fba'), textAlign: 'right' }} onClick={() => handleFbaSort('total_fba')}>
                          <span style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: '4px' }}>Total FBA <SortIcon col="total_fba" cur={fbaSortKey} dir={fbaSortDir} /></span>
                        </th>
